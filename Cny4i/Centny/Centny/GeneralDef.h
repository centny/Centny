...
#define FRAME_LOG(X) NSLog(@"Frame(%f,%f,%f,%f)",X.frame.origin.x,X.frame.origin.y,X.frame.size.width,X.frame.size.height)
//
#define IF_REL(X) if(X)[X release];X=nil
#define IF_REL_X(X,E) if(X){[X release];X=nil;E;}
#define IF_REL_ELSE(X,IX,EX) if(X){[X release];X=nil;IX;}else{EX;}
#define IF_E(X,IX) if(X){IX;}
#define IF_RETURN(X) if(X)return;
